...
namespace disassembler
{
	...
	uint8_t to_byte(std::string& str, size_t offset)
	{
		uint8_t b, n = 0;
	...
	void open(HANDLE handle, HMODULE _module)
	{
		...
		else
		{
			...
			// if you use this overload of the open function
			// (to perform operations in a separate program),
	...
	inst read(uintptr_t address)
	{
		...
		for (int opcode_at = 0; opcode_at < N_X86_OPCODES - 1; at = prev_at, opcode_at++)
		{
